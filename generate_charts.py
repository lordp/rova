...
def generate(year, week):
    if week > 1:
        prev_week = (
            ...
            .filter(Chart.week == week - 1, Chart.year == year, Chart.station == None)
        ...
        for station in urls:
            prev_week_stations[station] = (
                db.session.query(Chart)
                .filter(
                    Chart.week == week - 1, Chart.year == year, Chart.station == station
                )
                .order_by(Chart.position)
                .all()
            )
    ...
    rows = (
        db.session.query(Chart)
        .filter(Chart.week == week, Chart.year == year)
        .delete(synchronize_session=False)
    )
    db.session.commit()

    top_10 = (
        db.session.query(Artist.id, Song.id, db.func.count(Played.id).label("cnt"))
        .select_from(Played)
        .join(Song, Artist)
        .filter(
            db.func.date_part("week", Played.played_time) == week,
            db.func.date_part("year", Played.played_time) == year,
        )
    ...
    for station in urls:
        ...
        station_top_10 = (
            db.session.query(
                ...
                Song.id,
                Played.station,
                db.func.count(Played.id).label("cnt"),
            )
            .select_from(Played)
            .join(Song, Artist)
            .filter(
