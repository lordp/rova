...
def add_record(artist_name, song_name, song_length, played_time, station):
    ...
    song = (
        ...
        .filter(Song.name == song_name, Song.artist_id == artist.id)
        .first()
    )
    if not song:
        song = Song(
    ...
    play = (
        ...
        .filter(
            ...
            Played.song_id == song.id,
            Played.played_time == played_time,
...
rova_base_url = (
    "https://radio-api.mediaworks.nz/radio-api/v3/station/{station}/auckland/web"
)
rova_stations = {
    "edge": "theedge",
    "more": "morefm",
...
iheart_base_url = (
    "https://nz.api.iheart.com/api/v3/live-meta/stream/{station}/currentTrackMeta"
)
iheart_stations = {
    "zm": 6190,
    "hauraki": 6191,
...
for station, tag in rova_stations.items():
    try:
        ...
        played_time = (
            f"{j['nowPlaying'][0]['played_date']} {j['nowPlaying'][0]['played_time']}"
        )
        played_time = datetime.strptime(played_time, "%m/%d/%Y %H:%M:%S").astimezone(
            pytz.utc
        )
...
for station, tag in iheart_stations.items():
    try:
        ...
        played_time = datetime.fromtimestamp(j["startTime"] / 1000).astimezone(pytz.utc)
...
for song in songs:
    add_record(
        song["artist"], song["name"], song["length"], song["time"], song["station"]
    )
